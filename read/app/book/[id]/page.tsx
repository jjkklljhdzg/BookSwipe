...
export default function BookPage() {
    ...
    const handleAddToCollection = () => {
        ...
        if (isInCollection) {
            ...
            const newCollection = collectionArray.filter((id: number) => id !== book.id);
            localStorage.setItem('userCollection', JSON.stringify(newCollection));
        } else {
            // Добавляем в коллекцию
            collectionArray.push(book.id);
            localStorage.setItem('userCollection', JSON.stringify(collectionArray));
        }
    ...
    const handleSubmitComment = (e: React.FormEvent) => {
        ...
        if (!book || !userRating || !commentText.trim()) {
